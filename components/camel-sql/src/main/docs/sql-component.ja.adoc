[[sql-component]]
== SQL コンポーネント 
TODO 表記ゆれの統一 Component or コンポーネント

*Camel version 1.4から利用可能*

*sql:*コンポーネントによりJDBCクエリを使ってデータベースを操作する事ができる.このコンポーネントと<<jdbc-component,JDBC>>コンポーネントの違いには次のようなものがある.(SQLコンポーネントはエンドポイントのプロパティとしてSQLを指定できること, クエリにわたすパラメータをメッセージのペイロードに指定できること)

このコンポーネントは実際のSQLの操作のために`spring-jdbc`を内部で使用している.

Mavenユーザーはこのコンポーネントを使用するために, `pom.xml`内に以下の依存関係を追加する必要がある.
...
SQLコンポーネントは以下の機能をサポートする:

* JDBCをベースリポジトリとしたIdempotent Consumer EIP パターン(詳細は以下を参照)
* JDBCをベースリポジトリとしたAggregator EIP パターン(詳細は以下を参照)

=== URI format

警告:Camel2.11以降SQLコンポーネントはコンシューマーエンドポイントも(`from()`)プロデューサーエンドポイントも(`to()`)作成する事ができる.2.11より前のバージョンではプロデューサーとしてしか利用できない.

インフォ:SQLコンポーネントはhttp://camel.apache.org/transactional-client.html[Transactional Client]として使用する.


SQLコンポーネントのエンドポイントの書式は以下のように記述する.
...
Camel2.11以降のバージョンでは#`パラメータ名`という形式で名前付きパラメータを使用することができる.
...
Camelは名前付きパラメータの値を以下の順序で検索する
 1. `java.util.Map`形式のメッセージボディ
 2. メッセージヘッダー
 

名前付きパラメータを見つけることができなかった場合は、Camelは例外を発生させる.

*Camel2.14*以降ではシンプル式を利用することができる: TODO シンプル式は通じないかも?
...
#TODO ここの翻訳が問題ないか要確認
`?`はエンドポイントに対するオプションの指定で使用されるため,通常SQLクエリ内のパラメータを示す`?`は,`pass:[#]`で表現する.`?`の置き換えはエンドポイント単位で行われる.

*Camel 2.17*以降はSQLクエリをクラスパスもしくはファイルシステム上に外出しすることができる.
...
以下はクラスパス上に配置されたSQLファイル(myquery.sql)の記載例である.
...
SQLはエンドポイント上に直接指定するときとは異なって、行をまたいで記述してもよい.また--でコメントを記載することもできる.

以下のフォーマットでURIに対してオプションを指定することができる.
...
SQLコンポーネントは以下3つのオプションを指定できる.


[width="100%",cols="2,5,^1,2",options="header"]
|===
| オプション名 | 説明 | デフォルト値 | 型
| *dataSource* (共通) | データベースへ接続する際に使用するデータソースオブジェクトを指定する. |  | DataSource
| *usePlaceholder* (アドバンスト) | SQL内に記載されたプロパティプレースホルダーを全て置き換えるかどうかを指定する.. このオプションのデフォルト値はtrue. | true | boolean
| *resolveProperty Placeholders* (アドバンスト) | SQLコンポーネントが起動したときにプロパティプレースホルダーを置換するかどうかを指定する. ただし,値が文字列型のプレースホルダーのみ使用できる. | true | boolean
|===
...
SQLエンドポイントはURI形式で記述する:

----
sql:query
----

SQLエンドポイントはパスパラメータと複数のクエリパラメータで構成される:
...
|===
| パラメータ名 | 説明 | デフォルト値 | 型
| *query* | *必須* 実行したいSQLクエリを指定する. プレフィックス(file: もしくは classpath:)とパスを指定することで,SQLクエリをファイルとして外出しすることができる. |  | String
|===
